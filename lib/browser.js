...
const IDLE_CLOSE_MS = 10 * 1000; // 30 de secunde fara activitate
let idleTimer = null;
...
// ── Watchdog memorie ──
// Planul free Render are limita de 512MB. Dacă Node+Chromium se apropie de
// prag, închidem Chromium imediat (eliberează cea mai mare parte), înainte
// ca Render să omoare procesul cu OOM. Verificăm la 5 secunde.
const MEMORY_LIMIT_MB = 420; // prag de precauție (sub limita Render de 512MB)
...
function startMemoryWatchdog() {
  ...
  memoryTimer = setInterval(() => {
    ...
    if (rssMB > MEMORY_LIMIT_MB) {
      console.log(`[browser] ⚠️ RSS ${rssMB}MB peste prag (${MEMORY_LIMIT_MB}MB) — inchid Chromium preventiv.`);
      closePersistent().catch(() => {});
    }
  }, 2000);
  memoryTimer.unref?.();
...
async function getPersistentPage(opts = {}) {
  ...
  browserInstance = await chromium.launch({
    headless: true,
    channel: process.env.LOW_RAM === 'true' ? 'chromium-headless-shell' : 'chromium',
    args: [
      ...
      '--no-first-run',
      '--disable-extensions',
      // ── Optimizari RAM doar pe gazde sarace (Render free 512MB) ──
      // Pe GitHub Actions / PC normal acestea cauzeaza crash-uri.
      ...(process.env.LOW_RAM === 'true'
        ...
        : []),
      // Opreste lucrurile inutile care tin RAM.
      '--disable-dev-tools',
  ...
  contextInstance = await browserInstance.newContext({
    ...
    // Nu incarcam imagini/fonturi/media pe consola G4F — nu ne trebuie,
    // iar un screenshot full al consolei poate fi câteva MB de imagini.
    viewport: { width: 1280, height: 720 }
  });
  // Blocam resursele grele ca să ținem memoria jos pe consola G4F
  // (Livewire face polling continuu — fiecare răspuns poate aduce binar).
  // CSS-ul îl lăsăm să se încarce: waitFor visible pentru .time span și
  // .status-pill depinde de display:none din stylesheets.
...
// ── Citeste DAILY AD LIMIT din reward-center ──
// Navigheaza la https://control.gaming4free.net/reward-center si extrage
// "earned today / max" din textul "DAILY AD LIMIT". Returneaza null daca
// pagina nu se incarca sau daca datele lipsesc — atunci continuam fara
// safety check (tryWebAdExtend va esua natural la limita).
async function readDailyAdLimit(page) {
  if (process.env.LOW_RAM === 'true') {
    debug('[limit] Skip DAILY AD LIMIT check pe LOW_RAM (salveaza memorie).');
    return null;
  }
  const RESULT_URL = 'https://control.gaming4free.net/reward-center';
  const currentUrl = page.url();
  let navigated = false;
  try {
    if (!currentUrl.startsWith(RESULT_URL)) {
      await page.goto(RESULT_URL, { waitUntil: 'domcontentloaded', timeout: 30000 });
      navigated = true;
    }
  } catch (err) {
    debug(`[limit] Nu am putut naviga la reward-center: ${err.message.split('\n')[0]}`);
    return null;
  }

  try {
    // Asteptam sa apara blocul "DAILY AD LIMIT" (max 15s).
    await page.locator('text=DAILY AD LIMIT').first()
      .waitFor({ state: 'visible', timeout: 15000 }).catch(() => null);

    const data = await page.evaluate(() => {
      // Cauta textul care contine "earned today" si extrage cifrele.
      const all = document.body.innerText || '';
      const m = all.match(/\$?([\d.]+)\s*\/\s*\$?([\d.]+)\s*earned today/i);
      if (!m) return null;
      return {
        earnedToday: parseFloat(m[1]),
        maxDaily: parseFloat(m[2]),
      };
    });
    if (!data || isNaN(data.earnedToday) || isNaN(data.maxDaily)) {
      debug(`[limit] Nu am putut extrage DAILY AD LIMIT (pagina incarcata?=${navigated}).`);
      return null;
    }
    const remaining = Math.max(0, +(data.maxDaily - data.earnedToday).toFixed(2));
    debug(`[limit] Daily ad limit: $${data.earnedToday} / $${data.maxDaily} (mai sunt ~$${remaining}).`);
    return { earnedToday: data.earnedToday, maxDaily: data.maxDaily, remaining };
  } catch (err) {
    debug(`[limit] Eroare citire DAILY AD LIMIT: ${err.message.split('\n')[0]}`);
    return null;
  } finally {
    // Intoarcem browserul pe pagina initiala (console) daca am navigat
    // — altfel urmatoarele apeluri de pe page (extend) esueaza.
    if (navigated) {
      try {
        await page.goto(currentUrl, { waitUntil: 'domcontentloaded', timeout: 30000 });
        await page.locator('.time span').first().waitFor({ state: 'visible', timeout: 30000 });
      } catch (err) {
        debug(`[limit] Nu am putut reveni pe consola: ${err.message.split('\n')[0]}`);
      }
    }
  }
}
...
// Web plateste o reclama din 2 ($0.005 / $0.15 = ~15 pe zi).
// Continuam prin TOATE reclamele — semnalul de oprire e butonul dezactivat.
async function tryWebAdExtend(page, beforeSeconds) {
  const MAX_ADS = parseInt(process.env.WEB_AD_MAX || '15', 10);

  const startLimit = await readDailyAdLimit(page);
  if (!startLimit) {
    debug(`[extend] Calea 0: nu am putut citi DAILY AD LIMIT.`);
  } else if (startLimit.remaining <= 0) {
    debug(`[extend] Calea 0: limita zilnica atinsa ($${startLimit.earnedToday} / $${startLimit.maxDaily}) — sarim.`);
    return null;
  } else {
    debug(`[extend] Calea 0: DAILY AD LIMIT $${startLimit.earnedToday} / $${startLimit.maxDaily}.`);
  }

  let last = beforeSeconds;
  let used = 0;

  for (let i = 0; i < MAX_ADS; i++) {
    if (!page || page.isClosed()) {
      debug(`[extend] Pagina inchisa dupa ${used} reclame — pastram progresul.`);
      break;
    }
    ...
    try {
      const btnState = await page.evaluate(() => {
        const btn = document.querySelector('.rt-btn-free');
        if (!btn) return { ok: false, error: 'renewal-timer root negasit' };
        const disabled = btn.disabled || btn.hasAttribute('disabled') || btn.getAttribute('aria-disabled') === 'true';
        return { ok: true, disabled };
      });
      if (!btnState.ok) {
        debug(`[extend] ${btnState.error} — oprim ciclul.`);
        break;
      }
      if (btnState.disabled) {
        debug(`[extend] Butonul e dezactivat — limita zilnica sau cooldown atins.`);
        break;
      }
      ...
      if (!callResult.ok) {
        debug(`[extend] Apelul a esuat — oprim.`);
        break;
      }

      used++;
      debug(`[extend] Reclama ${used}/${MAX_ADS}: ${JSON.stringify(callResult)}`);

      // Asteptam 8s ca Livewire sa proceseze raspunsul XHR si sa updateze DOM-ul.
      // (Inainte foloseam waitForTimeIncrease cu 65s timeout — prea lung, creste RAM.)
      await page.waitForTimeout(8000);

      // Citim timer-ul curent — retry daca Livewire inca re-randeaza componenta
      let timerText = null;
      for (let retry = 0; retry < 3; retry++) {
        timerText = await page.locator('.time span').first()
          .textContent({ timeout: 5000 }).catch(() => null);
        if (timerText) break;
        await page.waitForTimeout(3000);
      }
      const timerSeconds = parseHms(timerText);
      if (timerSeconds !== null) {
        if (timerSeconds > last) {
          debug(`[extend] ✅ +${Math.round((timerSeconds - last) / 60)} min (total +${Math.round((timerSeconds - beforeSeconds) / 60)} min)`);
        }
        last = timerSeconds;
      } else {
        debug(`[extend] Reclama ${used}: timer N/A dupa ad call.`);
      }

      await page.waitForTimeout(2000);
    } catch (e) {
      debug(`[extend] Browser crash/inchis dupa reclama ${used}: ${String(e.message).split('\n')[0]}`);
      break;
    }
  }

  if (last !== null && beforeSeconds !== null && last - beforeSeconds > 0) {
    const mins = Math.round((last - beforeSeconds) / 60);
    debug(`[extend] ✅ Extindere finalizata: +${mins} min (${used} reclame). Timer: ${formatHms(last)}`);
    return {
      ok: true,
      action: 'extend',
      remainingSeconds: last,
      remainingLabel: formatHms(last),
      addedMinutes: mins,
      adsUsed: used,
    };
  }

  debug(`[extend] Calea 0 fara progres — continuam cu calea A/B`);
  return null;
}

function totalAddedZero(last, before) {
  return last === null || before === null || last - before <= 1800;
}
...
async function extendServer(minutes = 90) {
  ...
  return withLock(async () => {
    ...
    try {
      ...
      // Fix: pe Render reload-ul cu Livewire face timeout 60s (retea lenta/Cloudflare).
      // Daca .time span e deja vizibil, sarim complet peste reload - economisim 120s si RAM.
      let reloaded = false;
      const alreadyVisible = await page.locator('.time span').first().isVisible().catch(() => false);
      if (alreadyVisible) {
        debug(`[extend] .time span deja vizibil - sar peste reload`);
        reloaded = false;
      } else {
        for (let attempt = 1; attempt <= 2 && !reloaded; attempt++) {
          try {
            ...
            debug(`[extend] Reload attempt ${attempt} timeout: ${err.message.split('\n')[0]}`);
            if (attempt === 1) await page.waitForTimeout(3000);
          }
        }
      }
      debug(`[extend] URL dupa reload check (reloaded=${reloaded}): ${page.url()}`);
      ...
      try {
        ...
        debug(`[extend] .time span nu e vizibil - incerc fara reload, astept 15s`);
        await page.waitForTimeout(5000);
        await page.locator('.time span').first().waitFor({ state: 'visible', timeout: 30000 }).catch(() => {
          throw err;
        });
      }
...
async function renewServer() {
  ...
  return withLock(async () => {
    ...
    try {
      ...
      await renewBtn.click({ timeout: 10000, noWaitAfter: true });
      await sleep(3000);
      ...
      await confirmBtn.click({ timeout: 10000, noWaitAfter: true });
...
// .status-pill apare initial ca OFFLINE in HTML-ul paginii, iar Livewire il
// actualizeaza la ONLINE la primul poll (~3s). Daca citim imediat dupa un
// load/reload (ex. browserul e inchis la idle si se deschide o pagina fresh),
// raportam gresit OFFLINE. Re-citim de cateva ori: acceptam ONLINE la prima
// aparitie, OFFLINE doar daca se confirma la toate citirile.
async function readOnlineStatus(page) {
  for (let i = 0; i < 3; i++) {
    const text = await page
      ...
      .catch(() => '');
    if ((text || '').toUpperCase().includes('ONLINE')) return true;
...
async function readLiveStatus(page) {
  ...
  const rawOnline = await readOnlineStatus(page);
