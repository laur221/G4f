...
const IDLE_CLOSE_MS = 30 * 1000; // 30 de secunde fara activitate
let idleTimer = null;
...
const MEMORY_LIMIT_MB = 400; // prag de precauție (sub limita Render de 512MB)
...
// Verificam DAILY AD LIMIT inainte si dupa fiecare reclama — pe web, serverul
// accepta o reclamă din 2 ($0.005 / $0.15 = max ~15 reclame pe zi).
// Functia returneaza null pentru a lasa caiile A/B sa incerce.
async function tryWebAdExtend(page, beforeSeconds) {
  const MAX_ADS = parseInt(process.env.WEB_AD_MAX || '15', 10);

  // Pas 1: citim limita zilnica la inceput.
  const startLimit = await readDailyAdLimit(page);
  if (!startLimit) {
    debug(`[extend] Calea 0: nu am putut citi DAILY AD LIMIT — fallback la metodele clasice.`);
    return null;
  }
  if (startLimit.remaining <= 0) {
    debug(`[extend] Calea 0: limita zilnica atinsa ($${startLimit.earnedToday} / $${startLimit.maxDaily}) — sarim reclame.`);
    return null;
  }

  const estimatedMaxUseful = Math.ceil(startLimit.remaining / 0.005) * 2;
  const maxAds = Math.min(MAX_ADS, estimatedMaxUseful + 5);
  debug(`[extend] Calea 0: ~$${startLimit.remaining} ramasi — incercam maxim ${maxAds} apeluri...`);

  let last = beforeSeconds;
  let used = 0;
  let consecutiveUseless = 0;
  let lastEarnedToday = startLimit.earnedToday;
  ...
  for (let i = 0; i < maxAds; i++) {
    ...
    // Butonul dezactivat = limita atinsa sau cooldown.
    try {
      ...
      if (!btnState.ok) {
        debug(`[extend] ${btnState.error} — oprim ciclul de reclame.`);
        break;
      }
      if (btnState.disabled) {
        debug(`[extend] Butonul e dezactivat — limita/cooldown atins.`);
        break;
      }
      ...
      used++;
      debug(`[extend] Reclama ${used}/${maxAds}: ${JSON.stringify(callResult)}`);

      const check = await waitForTimeIncrease(page, last, 45000);
      if (!check.extended) {
        debug(`[extend] Reclama ${used} nu a marit timpul (${check.text?.trim() || 'N/A'}).`);
        consecutiveUseless++;
        if (consecutiveUseless >= 3) {
          debug(`[extend] 3 reclame consecutive fara castig — ne oprim.`);
          break;
        }
      } else {
        debug(`[extend] ✅ +${Math.round((check.seconds - last) / 60)} min (total +${Math.round((check.seconds - beforeSeconds) / 60)} min)`);
        consecutiveUseless = 0;
      }
      last = check.seconds !== null ? check.seconds : last;

      // Verificam DAILY AD LIMIT dupa fiecare reclama.
      const currentLimit = await readDailyAdLimit(page);
      if (currentLimit) {
        if (currentLimit.remaining <= 0) {
          debug(`[extend] Limita zilnica atinsa dupa ${used} reclame ($${currentLimit.earnedToday} / $${currentLimit.maxDaily}) — oprim.`);
          break;
        }
        if (currentLimit.earnedToday === lastEarnedToday) {
          consecutiveUseless++;
          debug(`[extend] Earned today neschimbat ($${currentLimit.earnedToday}) — serverul nu recunoaste apelul.`);
          if (consecutiveUseless >= 3) {
            debug(`[extend] 3 apeluri consecutive fara progres in earned today — oprim.`);
            break;
          }
        } else {
          lastEarnedToday = currentLimit.earnedToday;
          consecutiveUseless = 0;
          debug(`[extend] Earned today: $${currentLimit.earnedToday} / $${currentLimit.maxDaily}.`);
        }
      }
  ...
  if (last !== null && beforeSeconds !== null && last - beforeSeconds > 0) {
    // Rezultat construit local — functioneaza si daca pagina a murit intre timp.
