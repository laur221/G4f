...
// Web plateste o reclama din 2 ($0.005 / $0.15 = ~15 pe zi). Citim DAILY AD LIMIT
// o singura data la inceput — pentru log/info. Stop: buton dezactivat sau 2 reclame
// consecutive fara castig in timer.
async function tryWebAdExtend(page, beforeSeconds) {
  const MAX_ADS = parseInt(process.env.WEB_AD_MAX || '15', 10);

  // Citim DAILY AD LIMIT o singura data — doar pentru log, nu pentru stop.
  const startLimit = await readDailyAdLimit(page);
  if (!startLimit) {
    debug(`[extend] Calea 0: nu am putut citi DAILY AD LIMIT — folosesc ${MAX_ADS} reclame max.`);
  } else {
  ...
  for (let i = 0; i < MAX_ADS; i++) {
    ...
    // Butonul dezactivat = limita zilnica sau cooldown atinse.
    try {
      ...
      if (!check.extended) {
        ...
        consecutiveUseless++;
        if (consecutiveUseless >= 2) {
          debug(`[extend] 2 reclame consecutive fara castig — ne oprim.`);
          break;
        }
      } else {
