...
async function getPersistentPage(opts = {}) {
  ...
  browserInstance = await chromium.launch({
    ...
    args: [
      ...
      '--no-first-run',
      '--disable-extensions',
      // ── Reducere RAM (plan free Render: 512MB) ──
      '--js-flags=--max-old-space-size=96',
      '--memory-pressure-off',
      // Un singur proces pentru site-uri (în loc de unul per origin) —
      // cea mai mare economie de memorie la Chromium headless.
...
async function tryWebAdExtend(page, beforeSeconds) {
  ...
  if (last !== null && beforeSeconds !== null && last - beforeSeconds > 1800) {
    const result = await buildSuccessResult(page, beforeSeconds);
    result.adsUsed = used;
    return result;
  }
