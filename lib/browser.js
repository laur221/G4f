...
function scheduleIdleClose() {
  ...
  idleTimer = setTimeout(() => {
    ...
    // nu închidem browserul — reprogramăm.
    if (running) {
      scheduleIdleClose();
      return;
    }
...
async function tryWebAdExtend(page, beforeSeconds) {
  ...
  const check = await waitForTimeIncrease(page, beforeSeconds, 25000);
  if (check.extended) {
...
module.exports = { runAction, getStatus, extendServer, getPersistentPage };