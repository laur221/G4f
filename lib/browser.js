...
// Folosim TOATE reclamele disponibile (max WEB_AD_MAX, default 15) intr-un
// singur run, ca sa urcam timer-ul spre plafon (ex. 48h).
async function tryWebAdExtend(page, beforeSeconds) {
  const MAX_ADS = parseInt(process.env.WEB_AD_MAX || '15', 10);
  debug(`[extend] Calea 0 (gratuita): folosim pana la ${MAX_ADS} reclame...`);

  let last = beforeSeconds;
  let used = 0;

  for (let i = 0; i < MAX_ADS; i++) {
    // Daca browserul a crapat intre reclame, oprim si pastram progresul.
    ...
    try {
      // Butonul e pe cooldown / dezactivat? Atunci nu mai avem reclame disponibile.
      ...
      if (btnState.disabled) {
        debug(`[extend] Butonul e dezactivat dupa ${used} reclame — limita/cooldown atins.`);
        break;
      }
      ...
      // Livewire actualizeaza timer-ul cu intarziere — 45s ca sa prinda cresterea.
      const check = await waitForTimeIncrease(page, last, 45000);
      if (!check.extended) {
        debug(`[extend] Reclama ${used} nu a marit timpul (${check.text?.trim() || 'N/A'}) — probabil limita zilnica atinsa.`);
        if (used === 1 && totalAddedZero(last, beforeSeconds)) {
          // nici macar prima nu a mers -> lasam caii A/B sa incerce
          debug(`[extend] Calea 0 esuata complet — continuam cu calea A/B`);
          return null;
        }
        break;
      }
      ...
      // Pauza scurta intre reclame (Livewire proceseaza + anti-spam)
      await page.waitForTimeout(4000);
    } catch (e) {
      // Crash Chromium / pagina inchisa: pastram ce am castigat pana acum.
      debug(`[extend] Browser crash/inchis dupa reclama ${used}: ${String(e.message).split('\n')[0]}`);
      break;
    }
  }

  if (last !== null && beforeSeconds !== null && last - beforeSeconds > 1800) {
