...
async function extendServer(minutes = 90) {
  ...
  return withLock(async () => {
    ...
    try {
      ...
      const buttonTextBefore = (await btn.locator('span').textContent().catch(() => '')).trim();
      const isAdBased = buttonTextBefore.includes('watch ad') || buttonTextBefore.includes('ad');

      await btn.click();

      // Asteptam ca extinderea sa fie procesata
      // In headless (Render), Turnstile poate necesita mai mult timp sau nu se rezolva automat.
      // Asteptam pana la 40s si verificam daca timpul a crescut.
      let extended = false;
      ...
      while (Date.now() - startMs < 40000) {
        ...
        if (
          ...
          extended = true;
          console.log(`[extend] ✅ Extindere reusita! +${Math.round((currentSeconds - beforeSeconds) / 60)} min.`);
          break;
        }

        // Daca Turnstile a aparut si a trecut suficient timp, inseamna ca s-a rezolvat
        if (turnstileWasVisible && !tsVisible && Date.now() - startMs > 10000) {
          ...
          if (afterSeconds !== null && beforeSeconds !== null && (afterSeconds - beforeSeconds) > 1800) {
            extended = true;
            console.log(`[extend] ✅ Extindere reusita (Turnstile rezolvat)! +${Math.round((afterSeconds - beforeSeconds) / 60)} min.`);
            break;
          }
        }
      }
      ...
      if (!extended) {
        // Inchide Turnstile daca e inca deschis
        await page.evaluate(() => {
          if (typeof g4fCloseTurnstile === 'function') g4fCloseTurnstile();
        }).catch(() => {});

        // Verificam inca o data daca timpul a crescut dupa inchiderea Turnstile
        await page.waitForTimeout(3000);
        ...
        if (finalSeconds !== null && beforeSeconds !== null && (finalSeconds - beforeSeconds) > 1800) {
          extended = true;
          console.log(`[extend] ✅ Extindere reusita dupa inchiderea Turnstile! +${Math.round((finalSeconds - beforeSeconds) / 60)} min.`);
        }
      }
