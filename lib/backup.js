...
const { getPersistentPage } = require('./browser');
...
async function downloadSave(stagingDir) {
  ...
  const links = await withTimeout(listDownloadLinks(saveDirUrl), 90000, 'listare director save');
  const playerLinks = await withTimeout(listDownloadLinks(playersDirUrl), 90000, 'listare director Players');
  const all = [...links, ...playerLinks];
