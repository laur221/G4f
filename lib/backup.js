...
async function listDownloadLinks(dirUrl) {
  ...
  const origin = new URL(dirUrl).origin;
  // Pagina de fișiere Filament încarcă zeci de scripturi/trackere de pe alte
  // domenii (reclame etc.) — pe browserul slim din Render (single-process,
  // heap 96MB) partajat cu consola care face polling, ele întârzie
  // 'domcontentloaded' dincolo de timeout. Le blocăm: tabelul și link-urile
  // de download sunt toate same-origin (control.gaming4free.net).
  const blockCrossOrigin = async (route) => {
  ...
  await context.route('**/*', blockCrossOrigin);
  const tab = await context.newPage();
  try {
    await tab.goto(dirUrl, { waitUntil: 'domcontentloaded', timeout: 120000 });
    ...
    await tab.waitForSelector('a[href*="/files/download/"]', { state: 'attached', timeout: 60000 }).catch(() => {});
    await tab.waitForTimeout(2000);
    return await tab.evaluate(() =>
      [...new Set([...document.querySelectorAll('a[href*="/files/download/"]')].map((a) => a.href))]
    );
  } finally {
    await context.unroute('**/*', blockCrossOrigin).catch(() => {});
