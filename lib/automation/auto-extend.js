...
let TARGET_SECONDS = parseInt(process.env.AUTO_EXTEND_TARGET_SECONDS || '144000', 10);
let BACKUP_SECONDS = parseInt(
  process.env.AUTO_EXTEND_BACKUP_SECONDS || '7200', 10
);
const CHECK_INTERVAL_MS = 600000 // fortat 10min, ignora env Render (era 5min);
...
async function checkAndExtend() {
  ...
  try {
    ...
    // Server SUSPENDED (timer a ajuns la 0).
    // Renew-ul automat consuma prea multa memorie pe Render free (256MB)
    // si cauzeaza crash loop. Logam si asteptam renew manual din /admin.
    if (status.suspended) {
      console.log('[auto-extend] 🚨 Server SUSPENDED — renew manual necesar din /admin sau control.gaming4free.net');
      return;
    }
...
function startAutoExtend() {
  ...
  // delay prima verificare 45s ca Render sa stabilizeze memoria dupa pornire
  setTimeout(checkAndExtend, 90000); // 90s delay ca sa nu OOM la 45s
